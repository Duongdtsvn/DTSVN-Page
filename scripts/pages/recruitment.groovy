// Import các thư viện cần thiết để xử lý tìm kiếm tuyển dụng
import org.craftercms.sites.editorial.SearchRecruitments

// Khởi tạo đối tượng SearchRecruitments để thực hiện các thao tác tìm kiếm tin tuyển dụng
def searchRecruitments = new SearchRecruitments(searchClient, urlTransformationService)

// Lấy tham số trang hiện tại từ URL, mặc định là trang 1 nếu không có
def page = params.page ? params.page.toInteger() : 1
// Số lượng tin tuyển dụng hiển thị trên mỗi trang
def itemsPerPage = 12

// Tính toán vị trí bắt đầu để lấy dữ liệu cho trang hiện tại
def start = (page - 1) * itemsPerPage

// Lấy tham số ngôn ngữ, mặc định là tiếng Việt
def lang = params.lang ?: "vi"

// Khởi tạo biến để lưu trữ danh sách tuyển dụng và tổng số
...
templateModel.recruitmentItems = recruitmentItems    // Danh sách tuyển dụng cho trang hiện tại
...
templateModel.itemsPerPage = itemsPerPage          // Số tin trên mỗi trang
templateModel.language_s = lang                    // Ngôn ngữ hiện tại